...
jQuery(document).ready(function(){
	jQuery('input,select,textarea,button').each(function(){
		try{
			...
			if(!jQuery(this).hasClass('vj_noevent')  && !__frm.hasClass('vj_noevent') && _vj.trackForms){ //'vj_noevent' class can be set at the form or element levels, and will disable tracking
...
jQuery(function() {
	...
	try{
	    ...
	    if (bottom >= jQuery('#vj_doScroll').scrollTop() + jQuery('#vj_doScroll').innerHeight() && !endContent) { // use #vj_doScroll id on any object you want to detect
		currentTime = new Date();
		...
		if (!_vj.debug) {
		    ...
		    if(!attention){console.log("'_trackEvent', 'attention', "+location.pathname+"' : reader/scanner', "+totalTime+", undefined, false");}
		}
		didComplete = true;
	    }
