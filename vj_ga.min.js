...
var _vj = {
    ...
    ,utmhost:		'.' + _vj.arrhost[_vj.arrhost.length-2] + '.' + _vj.arrhost[_vj.arrhost.length-1]
};
...
~*~*~*~*~*~*~*~*~*~*~*~*~*/
try{
    if(_vj.userDefined != false && (_vj.cookeez.eat('__utmud',1) == false)){
	_gaq.push(['_setVar', _vj.userDefined]);
	//bake a cookie so we know we logged
	_vj.cookeez.bake('__utmud','init',false);
    }
} catch(err) { console.log(err); }
...
jQuery(function() {
	...
	try{
		...
		_gaq.push(['_trackEvent', 'viewport', state +' avail width', win.maxW.toString(), undefined, isPassive]);
	    }
...
(function (window, undefined) {
	...
	if(status){  //here, only tracking positives, and only as a "non-interaction" event (6th arg true)
	    ...
	    //logHistory += network.substring(0,2) + '~';
	    //window._gaq.push(['_setCustomVar', 5, 's', logHistory, 2]);
	}
    }
	...
	if(_vj.cookeez.eat('__utmvj',1) == false){
	    ...
	    //bake a cookie so we know we logged
	    _vj.cookeez.bake('__utmvj','init',false);
	}
    };
...
CV5 is used as a session var by the social media login detection later on
5.	(l33t "S" as in social) tilde ~ delimited string storing the first two chars of each network visitor is logged into
...
(function (window, undefined) {
	...
	,setCV: function(gaScope){
		...
		if (cv == false || cv[2] != id){ // ||cv4 neq id				 		
		    ...
		    gaScope.push(['_setCustomVar', 2, 'l', crop(window.location.pathname), 1]);
