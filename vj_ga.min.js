...
INTEGRATED 12/12
    ...
    multiple tracker support - currently losing referral info, but not poisoning the primary
...
(function (window, undefined) {
	...
	try{
	    ...
	    if(_vj.debug){
		console.log("'_trackEvent',"+ cat+","+ action+","+ label+", undefined, false");
	    }else{
		_gaq.push(['_trackEvent', cat, action, label, undefined, false]);
	    }
	} catch(err) {
...
~*~*~*~*~*~*~*~*~*~*~*~*~*/
try{
    if(_vj.userDefined !== false && _vj.userDefined.length > 0 && (_vj.cookeez.eat('_vj-ud') === false)){
	_gaq.push(['_setVar', _vj.userDefined]);
	//bake a cookie so we know we logged
	_vj.cookeez.bake('_vj-ud','init',false);
    }
} catch(err) { console.log(err); }
...
jQuery(document).ready(function(){
	jQuery('input,select,textarea,button').each(function(){
		try{
			...
			if(!jQuery(this).hasClass('gae_noevent')  && !__frm.hasClass('gae_noevent') && _vj.trackForms){ //'gae_noevent' class can be set at the form or element levels, and will disable tracking
	...
	jQuery("*").click(function(e){
		try{
			...
			if(!jQuery(e.target).parents().andSelf().hasClass('gae_noclick') && _vj._thisClickEvent != ePos && (typeof(_vj.trackClicks) === 'undefined' || _vj.trackClicks == true)){/* look up the chain for .gae_noclick, and only track if false*/
		...
		if(typeof(_vj.trackParams) !== 'undefined' && _vj.trackParams.length > 0 && window.location.search.length > 0){
			for(i=0; i<_vj.trackParams.length; i++){
				if(typeof(jQuery.querystring[_vj.trackParams[i]]) !== 'undefined' && _vj.paramsLogged.indexOf(_vj.trackParams[i])<0 ){								
					_evTrackProxy('params', _vj.trackParams[i], jQuery.querystring[_vj.trackParams[i]]);
...
jQuery(function() {
	...
	try{
	    if(state === 'resize' || _vj.cookeez.eat('_vj-lw') === false){  //only perform on first run
		...
		win.preW = win.width;
		win.preH = win.height;
		_vj.cookeez.bake('_vj-lv', 'init', false);
	    }
	}catch(err){
    ...
    function logWindow(){
	try{
	    if(_vj.cookeez.eat('_vj-lw') === false){  //only perform on first run  // not tracking height to keep events down - easy enough to pull from "screen resolution" after segmenting viewport width
		win.maxW = window.screen.availWidth;
		if(typeof(_vj.debug) !== 'undefined' && _vj.debug === true){
		    console.log('avail width '+win.maxW);
		}else{
		    _gaq.push(['_trackEvent', 'viewport', 'avail width', win.maxW.toString(), undefined, true]);
		}
		_vj.cookeez.bake('_vj-lw', 'init', false);
	    }
	}catch(err){
...
(function (window, undefined) {
    ...
    ,beacons = { 
	     ...
	     *
	     * NOTE: the first two chars of a beacon will be passed to custom vars, please ensure they are unique
	     * 
	     */
	    facebook: { 
	...
	if(_vj.cookeez.eat('_vj-sd') == false){
	    ...
	    //bake a cookie so we know we logged
	    _vj.cookeez.bake('_vj-sd','init',false);
	}
    };
...
(function (window, undefined) {
	...
	,setCV: function(gaScope){
		...
		if (cv == false || cv[2] != id){ // ||cv4 neq id
		    if(_vj.debug){
			console.log("'_setCustomVar', 1, 'r', "+(document.referrer.length > 0)?_crop(document.referrer.substr(7,document.referrer.length)):"'(direct)'" +", 1");
