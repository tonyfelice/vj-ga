...
jQuery(function() {
    ...
    function logWindow(){
	try{
	    if(_vj.cookeez.eat('_vj-lw') === false){  //only perform on first run
		win.maxW = window.screen.availWidth;
		win.maxH = window.screen.availHeight;
		if(typeof(_vj.debug) !== 'undefined' && _vj.debug === true){
		    console.log('avail width '+win.maxW);
		    console.log('avail height '+win.maxH);
		}else{
		    _gaq.push(['_trackEvent', 'viewport', 'avail width', win.maxW.toString(), undefined, true]);
		    _gaq.push(['_trackEvent', 'viewport', 'avail height', win.maxH.toString(), undefined, true]);
		}
