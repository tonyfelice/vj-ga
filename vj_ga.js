...
*/
 (function(window, undefined) {
    ...
    var cookeez = {
        ...
        eat: function(name) {
            //returns boolean set/not; add second arg to return value as a str
            var nameEQ = name + "=";
...
// event proxy function to catch from "forms and clicks"
 (function(window, undefined) {
    ...
    //promote the fn - also exposed to flash, so please do not change fn name, and make sure this remains at window scope
    } (window));
    ...
    jQuery("*").click(function(e) {
        try {
            ...
            if (!jQuery(e.target).parents().andSelf().hasClass('vj_noclick') && _vj._thisClickEvent != ePos && (typeof(_vj.trackClicks) === 'undefined' || _vj.trackClicks == true)) {
                /* look up the chain for .vj_noclick, and only track if false*/
...
jQuery(function() {
    ...
    function logWindow() {
        try {
            if (_vj.cookeez.eat('_vj-utm-lw') === false) {
                //only perform on first run  // not tracking height to keep events down - easy enough to pull from "screen resolution" after segmenting viewport width
    ...
    if (_vj.trackViewport) {
        ...
        jQuery(window).resize(function() {
            ...
            //note: this timer is dormant UNLESS there's a resize event in play
            });
    }
});
 ...
 */
 (function(window, undefined) {
    ...
    logStatus = function(network, status) {
        if (status) {
            //here, only tracking positives, and only as a "non-interaction" event (6th arg true)
    ...
    inits = function() {
        ...
        if (_vj.cookeez.eat('_vj-utm-sd') == false) {
            ...
            for (var name in beacons) {
                beacon = beacons[name];
                if (beacon.appId) {
                    //fb is defined, but will not fire unless appId is handed to the fn when called by window
                    loadScr(beacon.src);
                    beacon.init(name);
...
if (_vj.trackSocial) {
    ...
    // add slight delay to avoid overflowing the event buffer (10 events onload, replenish at 1 per sec https://developers.google.com/analytics/devguides/collection/other/limits-quotas)
    }
...
*/
 (function(window, undefined) {
    ...
    var cvTools = {
        getCky: {},
        // extension hook; this lib needs cookie getter, but doesn't include it, needs to be extended with a bakery/cookie lib ex: _vj.cvTools.getCky = _vj.cookeez.eat;

        crop: function(str) {
            // cv str size limit is 64, ensure that we're not passing something too long
            var out,
            sz = 63;
        ...
        setGoal: function(gaScope, nom, val) {
            try {
                //fire event 'auto-goal'; set CV3 with: count.total.sincelast.timenow.timefirst
                var cv = this.getCV(3),
                dt = new Date().getTime();
                val = Math.round(val);
                //round to dollars, looking for the big picture here, and full string is dot delim so decimal in values would conflict.
...
if (_vj.customVars) {
    ...
    // give a little time for the cookies to bake
    }
